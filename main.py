...
def checkAudioExtension():
    ...
    for subFolder in glob.glob("*/"):
        ...
        if len(extensionsInFolder) != 1:
            errorLog.write("Folder has Audio Files with Different Extensions: %s" % subFolder[:-1])
...
def checkMP3(subFolder):
    ...
    if len(checkBitRate) == 1:
        ...
        errorLog.write("Folder has Audio Files with Different Qualities: %s" % subFolder[:-1])
        shutil.move(subFolder, "[00] Error")
    ...
    if len(checkBitsPerSample) == 1 and len(checkSampleRate) == 1:
        if "MQA" in str(audio.get("encoder")):
            if (audio.info.channels == 2):
                ...
                dest = subFolder + " [MQA][%ich][%i-%s]" % (
                audio.info.channels, checkBitsPerSample[0], checkSampleRate[0])
                os.rename(subFolder, dest)
        else:
            if (audio.info.channels == 2):
...
def renameFolders():
    ...
    try:
        ...
        errorLog.write("%s: %s" % (folderData["folderName"]), e)
